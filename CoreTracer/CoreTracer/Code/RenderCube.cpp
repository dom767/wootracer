...
bool DRenderCube::InternalIntersect(const DRayContext& rRayContext, DCollisionResponse& out_Response) const
{
	...
	if (t0>0 && !((rRayContext.m_RequestFlags&RequestBackface)>0))
	{
		...
		out_Response.mObjectPosition = start + (rRayContext.m_Ray.GetDirection()*t0); // object space
		out_Response.mNormal = normal0;
		return true;
	}
	if (t1>0 && ((rRayContext.m_RequestFlags&RequestBackface)>0))
	{
		if (t1<0.00001f)
			return false;
		out_Response.mObjectPosition = start + (rRayContext.m_Ray.GetDirection()*t1); // object space
		out_Response.mNormal = normal1.GetNegate();
		return true;
	}
