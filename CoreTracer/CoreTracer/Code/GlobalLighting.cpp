...
void DGlobalLighting::GetSceneryColour(const DRay& Ray, DColour& Colour) const
{
	switch(mMode)
	{
	case PlainColour:
...
void DGlobalLighting::Read(TiXmlElement* element)
{
//	Convert::StrToVec3(element->Attribute("position"), mPosition);
	Convert::StrToCol(element->Attribute("AmbientColour"), mAmbientColour);
	Convert::StrToCol(element->Attribute("BackgroundColour"), mBackgroundColour);
	int intMode;
	Convert::StrToInt(element->Attribute("BackgroundType"), intMode);
	mMode = (intMode == 1) ? PlainColour : Sky;
}